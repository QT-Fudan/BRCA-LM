copyKAT<- function (rawmat = rawdata, id.type = "S", cell.line = "no", 
          ...
          norm.cell.names = "", KS.cut = 0.1, sam.name = "", 
          ...
          plot.genes = "TRUE", genome = "hg20", n.cores = 1) 
{
  ...
  print(paste(nrow(rawmat), " genes, ", ncol(rawmat), 
              " cells in raw data", sep = ""))
...
#  }
  ...
  if (cell.line == "yes") {
    ...
    if (WNS == "unclassified.prediction") {
      basa <- baseline.GMM(CNA.mat = norm.mat.smooth, max.normal = 5, 
                           mu.cut = 0.05, Nfraq.cut = 0.99, RE.before = basa, 
                           n.cores = n.cores)
      basel <- basa$basel
      WNS <- basa$WNS
      preN <- basa$preN
...
#  }
  ...
  if (genome == "hg20") {
    ...
    if (cell.line == "yes") {
      ...
      if (distance == "euclidean") {
        jpeg(paste(sample.name, "heatmap.jpeg", 
                   sep = ""), height = h * 250, width = 4000, 
             res = 100)
        heatmap.3(t(mat.adj), dendrogram = "r", 
                  ...
                  symm = F, symkey = F, symbreaks = T, cex = 1, 
                  main = paste(WNS1, "; ", WNS, sep = ""), 
        ...
        if (plot.genes == "TRUE") {
          ...
          rbPal1g <- colorRampPalette(c("black", 
                                        "grey"))
          ...
          heatmap.3(t(results.com), dendrogram = "r", 
                    ...
                    cex = 1, main = paste(WNS1, "; ", WNS, 
                                          sep = ""), cex.main = 4, margins = c(10, 
                                                                               10))
          dev.off()
        }
      }
      ...
      if (distance == "euclidean") {
        ...
        heatmap.3(t(mat.adj), dendrogram = "r", 
                  ...
                  main = paste(WNS1, "; ", WNS, sep = ""), 
                  cex.main = 4, margins = c(10, 10))
        legend("topright", paste("pred.", 
                                 names(table(com.preN)), sep = ""), pch = 15, 
      ...
      if (output.seg == "TRUE") {
        ...
        colnames(Short) <- c("ID", "chrom", 
                             "loc.start", "loc.end", "num.mark", 
                             "seg.mean")
        head(Short)
      ...
      reslts <- list(res, cbind(Aj$RNA.adj[, 1:3], mat.adj), 
                     hcc)
      names(reslts) <- c("prediction", "CNAmat", 
                         "hclustering")
      return(reslts)
    }
  }
